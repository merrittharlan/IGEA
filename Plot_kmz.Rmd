...
for(i in 1:length(input_kmz)){
  unzip(paste0('KMZ_files/', input_kmz[i]),)
  file_id = substr(input_kmz[i], 1, 8)
  file_df = data.frame()
  kml_files[[i]] = read_sf(paste0(file_id, ".kml"))
  kml_coords = data.frame(getKMLcoordinates(paste0(file_id, ".kml"))[[1]])
...
```{r process, echo=TRUE}
ts_data = read.csv("TS_Practice_8_1_2021.csv")
head(ts_data)

gnss_data = read.csv("GNSS_points.csv")
head(gnss_data)
